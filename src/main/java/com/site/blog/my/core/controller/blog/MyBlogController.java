...
public class MyBlogController {
    ...
    public String page(HttpServletRequest request, @PathVariable("pageNum") int pageNum) {
        ...
        request.setAttribute("blogPageResult", blogPageResult); // 要显示的数据
        request.setAttribute("newBlogs", blogService.getBlogListForIndexPage(1));
        request.setAttribute("hotBlogs", blogService.getBlogListForIndexPage(0));
        request.setAttribute("hotTags", tagService.getBlogTagCountForIndex()); // 拿到前20条的BlogTagCount
    ...
    public String detail(HttpServletRequest request, @PathVariable("blogId") Long blogId, @RequestParam(value = "commentPage", required = false, defaultValue = "1") Integer commentPage) {
        BlogDetailVO blogDetailVO = blogService.getBlogDetail(blogId); // 返回已发布的blogId对应的blog信息
        if (blogDetailVO != null) {
            request.setAttribute("blogDetailVO", blogDetailVO); // blog信息(VO)
            request.setAttribute("commentPageResult", commentService.getCommentPageByBlogIdAndPageNum(blogId, commentPage)); // comment信息
        }
        request.setAttribute("pageName", "详情");
        request.setAttribute("configurations", configService.getAllConfigs()); // 获取网站基础配置信息
    ...
    public String search(HttpServletRequest request, @PathVariable("keyword") String keyword, @PathVariable("page") Integer page) {
        ...
        request.setAttribute("newBlogs", blogService.getBlogListForIndexPage(1)); // 1 代表按照更新时间搜索
        request.setAttribute("hotBlogs", blogService.getBlogListForIndexPage(0)); // 0 代表按热度搜索
        request.setAttribute("hotTags", tagService.getBlogTagCountForIndex()); // 获取所有的BlogTagCount信息
        request.setAttribute("configurations", configService.getAllConfigs()); // 获取网站基础配置信息
        return "blog/" + theme + "/list";
    }
    ...
    public String link(HttpServletRequest request) {
        ...
        Map<Byte, List<BlogLink>> linkMap = linkService.getLinksForLinkPage(); // 按类型将blogLink进行分组
        if (linkMap != null) {
