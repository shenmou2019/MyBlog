...
public class CommentServiceImpl implements CommentService {
    ...
    public Boolean reply(Long commentId, String replyBody) {
        ...
        if (blogComment != null && blogComment.getCommentStatus().intValue() == 1) {
            ...
            return blogCommentMapper.updateByPrimaryKeySelective(blogComment) > 0; // 更新评论
        }
        return false;
    }
    ...
    public PageResult getCommentPageByBlogIdAndPageNum(Long blogId, int page) {
        ...
        params.put("commentStatus", 1);// 过滤已审核的数据
        PageQueryUtil pageUtil = new PageQueryUtil(params);
        List<BlogComment> comments = blogCommentMapper.findBlogCommentList(pageUtil); // 查出相关评论
        if (!CollectionUtils.isEmpty(comments)) {
            int total = blogCommentMapper.getTotalBlogComments(pageUtil); // 查出评论数量
            PageResult pageResult = new PageResult(comments, total, pageUtil.getLimit(), pageUtil.getPage());
            return pageResult;
        }
        return null;
