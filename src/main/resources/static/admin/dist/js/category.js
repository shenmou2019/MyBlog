...
$('#saveButton').click(function () {
    var categoryName = $("#categoryName").val(); // categoryName
    if (!validCN_ENString2_18(categoryName)) {
        $('#edit-error-msg').css("display", "block");
        $('#edit-error-msg').html("请输入符合规范的分类名称！");
    } else {
        var params = $("#categoryForm").serialize(); // 序列化表单值:categoryName=xxx&categoryIcon=xxx
                ...
                $.ajax({
                    ...
                    success: function (r) {
                        if (r.resultCode == 200) {
                            ...
                            $("#jqGrid").trigger("reloadGrid"); // 删除成功，重新加载数据
                        } else {
                            swal(r.message, {
                                icon: "error",
                            });
