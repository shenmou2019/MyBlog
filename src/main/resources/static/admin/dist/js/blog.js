...
function editBlog() {
    var id = getSelectedRow(); // 获取选中行(只能为一行)
    if (id == null) {
        return;
    }
...
function deleteBlog() {
    var ids = getSelectedRows(); // 获取选中行(可以为多行)
    if (ids == null) {
        return;
    }
    swal({
        ...
        dangerMode: true,
            ...
            if (flag) { // 点击确认按钮，异步处理请求
                $.ajax({
                    type: "POST",
                    url: "/admin/blogs/delete",
                    contentType: "application/json",
                    data: JSON.stringify(ids), // 将ids转换为json字符串，传到后台处理(后台可以通过@RequestBody接收)
                    success: function (r) {
                        if (r.resultCode == 200) {
                            swal("删除成功", {
                                icon: "success",
                            });
                            $("#jqGrid").trigger("reloadGrid"); // 重新加载列表
                        } else {
                            swal(r.message, {
                                icon: "error",
                            });
