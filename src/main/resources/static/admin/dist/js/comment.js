...
$('#saveButton').click(function () {
    var replyBody = $("#replyBody").val(); // 回复评论的内容
    if (!validCN_ENString2_100(replyBody)) {
        ...
        var id = getSelectedRow(); // 选中的一条评论
        var params = {"commentId": id, "replyBody": replyBody}
        $.ajax({
            type: 'POST',//方法类型
            url: url,
            data: params, // params为一个对象，传到后台时，可以直接用@RequestParam接收，注意属性值一一对应
            success: function (result) {
                if (result.resultCode == 200) {
                    $('#replyModal').modal('hide'); // 成功后，隐藏模态框
                    swal("回复成功", {  // 弹出回复成功消息框
                        icon: "success",
                    });
                    reload(); // 重新刷新页面数据
